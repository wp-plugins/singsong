...
<h3>Settings</h3>
Please enter the full URL of the music file (or playlist) for each section: 
...
function singsong_check() {
	...
	if ((!empty($sshome)) && (is_home())) $filepath = trim($sshome);
	if ((!empty($sssingle)) && (is_single())) $filepath = trim($sssingle);
	if ((!empty($sspage)) && (is_page()))  $filepath = trim($sspage);
	if ((!empty($ssarchive)) && (is_archive())) $filepath = trim($ssarchive);
if ( ((!empty($sshome)) && (is_home())) || ((!empty($sssingle)) && (is_single())) || ((!empty($sspage)) && (is_page())) || 
((!empty($ssarchive)) && (is_archive())) )
{ 
?>
